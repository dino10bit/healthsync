...
The following dashboards will be created and maintained in Grafana and Firebase. Release events will be automatically added as annotations to all Grafana dashboards via a **GitHub Action that calls the Grafana API** on every production deployment.
...
To ensure consistency, all custom backend metrics will be published to the **`SyncWell/Backend`** CloudWatch namespace using the **Embedded Metric Format (EMF)** from within the Lambda/Fargate business logic.
...
| `TimeSinceLastWebhook`| `Provider` | Seconds | A gauge metric published by a scheduled Lambda that checks the timestamp of the last received webhook for each provider. |
...
| **Version Adoption**| Donut Chart | `analytics.users_by_app_version` | > 90% on latest version within 14 days | This ensures that we can deprecate older API versions and client builds in a timely manner, reducing maintenance overhead. |
...
| Alert Name | Metric | Threshold | Statistic | Period | Eval Periods |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **High API Gateway P99 Latency**| `Latency` (API Gateway) | > 500ms | p99 | 5 min | 2 |
| **API Gateway 5xx Error Spike**| `5xxError` | > 1% | Sum | 5 min | 1 |
| **Lambda Error Rate High**| `Errors` / `Invocations` | > 2% | Average | 5 min | 2 |
| **SQS Queue Depth High** | `ApproximateNumberOfMessagesVisible` | > 1000 | Maximum | 10 min | 3 |
| **DLQ Has Messages** | `ApproximateNumberOfMessagesVisible` (DLQ) | > 0 | Sum | 1 min | 1 |
| **DynamoDB Throttling** | `ReadThrottleEvents` or `WriteThrottleEvents` | > 10 | Sum | 5 min | 1 |
