...
To ensure consistency and quality as we scale to dozens of integrations, each third-party integration will be built against a standardized set of interfaces and utilities. This framework, maintained as a **Kotlin Multiplatform (KMP) shared module** named `integration-framework` located at `/shared/integration-framework` in the monorepo, separates the unique business logic of an integration from the boilerplate code required for all integrations. The module is owned by the **Core Backend team**, and its versioning is tied to the main application's release cycle.
...
*   **Centralized Error Handling:** A common set of exceptions that provider-specific code can throw to signal specific outcomes to the sync engine. While providers can throw other standard exceptions, they are encouraged to use these for predictable error handling by the sync engine.
...
| `RateLimitError` | `retryAfterSeconds: Int?`| Thrown when a rate limit is hit (e.g., 429 Too Many Requests), signaling the worker to back off. If `retryAfterSeconds` is null, the sync engine will use its default exponential backoff algorithm. |
...
*   **Shared Utilities:** Includes common functions for tasks like webhook signature verification to ensure consistency.
...
**Note on Complexity:** This is a comprehensive interface. For integrations that only support a subset of functionality (e.g., read-only), a future iteration may introduce smaller, more focused interfaces (e.g., `ReadableDataProvider`). This work is tracked in ticket `TECH-DEBT-567`. For the MVP, all methods must be implemented, even if they simply throw a `NotImplementedError`.
 ...
 * Each claimed capability must be validated by a set of automated certification tests.
 */
...
data class WebhookPayload(
    val userId: String,
    // Must be one of the keys from the canonical data models defined in 06-technical-architecture.md (e.g., "workout", "sleep_session").
    val dataType: String,
...
interface DataProvider {
     ...
     * For MVP, any casting or processing error must cause the entire job to fail by throwing an exception, aligning with the "all-or-nothing" sync strategy.
     */
     ...
     * Implementers should return null only if the provider's API does not support a lightweight check.
     ...
     * verifying the webhook's authenticity using the shared `WebhookVerifier` utility.
     */
...
/**
 * Defines a simple date range using a full ISO 8601 timestamp with timezone. Fractional seconds are supported.
 */
data class DateRange(
    val startDate: String, // e.g., "2023-01-01T00:00:00.123Z"
    val endDate: String    // e.g., "2023-12-31T23:59:59.456Z"
)
```
...
The process for updating the allow-list is to submit a Pull Request to the infrastructure-as-code (Terraform) repository, using the `firewall-change-request.md` template located in the `.github` directory. The PR must be reviewed and approved by at least one member of the **Security team** and one member of the **Core Backend team**.

## 3. Authentication: A Secure Hybrid Flow

The primary authentication flow assumes **OAuth 2.0 Authorization Code Flow with PKCE**. The key security principle is that **long-lived tokens never touch the user's device**. For providers that use other schemes (e.g., static API keys), a separate, audited, non-user-facing flow will be used where an administrator provisions the key directly into AWS Secrets Manager.
...
4.  **Redirect with Auth Code (Mobile):** The provider redirects to SyncWell's single, canonical redirect URI (`syncwell://oauth-callback`) with a one-time `authorization_code`. The `state` parameter is a short-lived, signed JWT (using **HS256**) created by the SyncWell backend, containing the `providerId` and a CSRF token.
...
Token management is a purely backend process. The shared module will automatically perform a pre-flight check for token validity and handle the refresh flow if necessary. If a refresh fails, it will throw a `PermanentAuthError`, which triggers the user notification flow defined in `17-error-handling.md`.
...
*   **`PermanentAuthError` Notification:**
    ...
    *   **Deep-link:** The deep-link will follow the schema `syncwell://reconnect?connectionId={connectionId}`. The mobile application's deep-linking router documentation contains the implementation details for handling this link.
...
Proactively managing third-party rate limits is a core architectural requirement. The strategy is to use a **distributed, global rate limiting** system using the **token bucket algorithm**, implemented in Redis. The initial bucket size for each provider will be set to its documented request limit per minute.
...
| **Fitbit** | Steps | `1/user/-/activities/steps/date/[date]/1d.json` | `N/A` | Cloud-to-Cloud (Webhook-First) | This is a self-imposed limitation for MVP. The Fitbit API supports writing data, but we are focusing on read-only for the initial release. |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **R-19** | An API provider makes a breaking change, disabling a key integration. | High | High | Implement contract testing using **Pact**. Have robust monitoring to detect API errors quickly. | Core Backend Lead |
...
| **R-22** | A vulnerability in the backend leads to a leak of user OAuth tokens from Secrets Manager. | Low | Critical | Enforce strict IAM policies with the principle of least privilege, including dynamic session policies for workers. Encrypt all secrets. Conduct regular security audits and penetration testing. | Security Lead |
| **R-24** | A bug in a single `DataProvider` implementation leaks user tokens. | Low | Critical | Rigorous code reviews, static analysis for security hotspots, and the use of dynamic, narrowly-scoped IAM session policies to prevent a `DataProvider` from accessing tokens it is not authorized for. | Security Lead |
| **R-25** | Inconsistent `DataProvider` implementations lead to varied quality and behavior. | Medium | Medium | The shared module mitigates this, but a strict, automated QA certification suite for new providers is essential. This suite must include tests for functionality, performance, and adherence to security best practices. | QA Lead |
...
*   **Circuit Breaker Pattern:** For notoriously unstable APIs, a Circuit Breaker pattern will be implemented within the shared module.
    ...
    *   **Tuning Process:** The default configuration is a starting point. The process for tuning it for a specific provider is to run a load test against a mock of that provider's API, gradually increasing the error rate and latency to find the optimal thresholds.
...
*   **Graceful Degradation via Feature Flags:** If a provider's API is causing persistent problems, a remote feature flag will be used to gracefully degrade or disable the integration.
    *   **Flags:** `integrations.{providerKey}.enabled` (boolean) and `integrations.{providerKey}.readOnly` (boolean). These flags can be modified in production by principals with the `iam:syncwell-admin` permission.
    ...
    *   **User-Facing Text (Disabled):** If an integration is disabled, it will be greyed out in the connection list with a label: `This integration is temporarily unavailable due to an issue with the provider.`
*   **Defensive Coding and Documentation:** The developer responsible for an integration is also responsible for documenting any known quirks or undocumented behaviors of the provider's API, using the `PROVIDER_QUIRKS.md` template located in the provider's source code directory.
