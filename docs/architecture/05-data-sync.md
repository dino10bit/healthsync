...
*   **Webhook-Driven Sync:** The preferred, most efficient method. To prevent "event chatter", an "Event Coalescing" layer (detailed in `06-technical-architecture.md`) buffers and merges rapid-fire webhooks. "Event chatter" is quantitatively defined as **more than 3 events for the same user within a 60-second window**. This definition was derived from an analysis of webhook traffic patterns from our beta providers, which showed that legitimate, distinct user actions are rarely clustered more tightly than this.
*   **Mobile-Initiated Sync:** For manual syncs, the API accepts a `priority` field (`high`, `medium`, `low`). This does not use a separate SQS queue, but instead provides a hint to the distributed rate-limiting engine to prioritize these jobs' access to the third-party API budget. If an invalid value is provided for the `priority` field, the API will return a `400 Bad Request` error.
*   **Tiered & Adaptive Polling:** For providers without webhooks, a highly cost-effective, two-tiered polling strategy is used:
    *   **Tier 1: Adaptive Scheduling:** After a job completes, the next poll is scheduled dynamically by enqueuing a message to an SQS queue with a calculated `DelaySeconds`. The delay is based on the user's "sync velocity", calculated as `velocity = (number of new items in last 7 days) / 7`. An active user might be re-queued with a 15-minute delay, while an inactive user might be re-queued with a 24-hour delay.
    *   **Tier 2: Pre-flight Check:** The scheduled message triggers a lightweight `PollingPreflightChecker` Lambda. This function performs a cheap API call (e.g., an HTTP `HEAD` request or a query for `count > 0`) to check for new data. Only if new data exists does it enqueue a job for the main `WorkerLambda` fleet, avoiding the cost of a full sync invocation for empty polls. If the pre-flight check fails due to a transient error, it will be retried up to 3 times with exponential backoff. If it fails permanently, an alert is sent to the on-call engineer.
...
4.  **Handle Empty Source:** If the `fetchMetadata` call succeeds and the returned list is empty, the job is considered complete with "no new data". The worker updates the `lastSyncTime`, and the SQS message is deleted. If the API call itself fails, the job is retried. This prevents `lastSyncTime` from being updated on a transient source API failure.
...
8.  **Intelligent Hydration:** If the list of `sourceRecordIds` is not empty, the worker calls `fetchPayloads(recordIds: ...)` on the source `DataProvider` to fetch the full, heavy data payloads **only for the records that will actually be written**.
    *   **Failure Handling:** If a payload fetch from the source fails for a specific record, that record is marked as failed and the process continues with the other records. This "per-record failure" approach is a post-MVP enhancement. For MVP, if any payload fetch fails, the entire job fails to prevent partial data writes. This is a known limitation that trades granularity for simplicity.
9.  **Write Data:** The worker calls `pushData(data: ...)` on the destination provider.
10. **Handle Partial Failures:** The worker inspects the `PushResult`. For the MVP, if the push is not 100% successful, the entire job is considered failed. The worker throws an error, allowing SQS to retry the job. This "all-or-nothing" approach for the MVP is inconsistent with the `PushResult` data model, but is a deliberate simplification. The post-MVP strategy, which will track per-item failures, is captured in `../prd/45-future-enhancements.md`.
...
*   **Configuration:** This value is managed in AWS AppConfig under the key `features/sync/conflict-detection-threshold-seconds` and has an acceptable range of **10 to 300 seconds**.
...
*   **`newest_wins`:** The data record with the most recent modification timestamp is preserved.
    *   **Tie-Breaking Rule:** If modification timestamps are identical, the **source record wins** to ensure deterministic behavior. A `DEBUG` level log will be generated when this tie-breaking rule is applied.
...
*   **Redis Implementation:** A Redis **String** is used for the counter.
    ...
    *   **TTL:** The key will have a **7-day TTL** to prevent stale data for inactive users.
*   **Logic:**
    1.  **Check Counter:** Before fetching from the destination, the worker checks the counter. If it exceeds a threshold (configurable in AppConfig, default **10**), the destination API call is skipped.
...
*   **Resilience:**
    ...
    *   **Stale Cache:** The risk of a stale cache causing a missed sync is low and self-correcting. If a `pushData` operation fails due to a conflict that would have been caught, the confidence counter is immediately reset to zero, forcing a full re-fetch on the next attempt. This resilience mechanism relies on the assumption that destination APIs will correctly return a conflict error, which must be validated for each provider implementation.
...
*   **Durable Queueing & DLQ:** The use of SQS for the `HotPathSyncQueue` provides a durable buffer for all jobs. A corresponding Dead-Letter Queue (DLQ) is configured to catch messages that fail repeatedly.
    *   **`maxReceiveCount`:** Set to **5**. This provides a balance between retrying transient errors and quickly detecting persistent failures.
    *   **`VisibilityTimeout`:** Set to **90 seconds**. This is based on the observed P95 latency of ~60 seconds for a single sync job, providing a safe buffer.
...
*   **User Communication:**
    *   **SLA:** The user MUST be contacted via email within **24 hours for Pro users** and **72 hours for Free users**.
    ...
    *   **Email Templates:** Pre-defined, user-friendly email templates are used. These templates are maintained in the company's customer engagement platform (e.g., Intercom) and support localization. The **Support Team Lead** is responsible for maintaining these templates.
...
This diagram details the step-by-step interaction between the `WorkerLambda` and other services during a single, successful delta sync job, including the "Intelligent Hydration" workflow.
...
sequenceDiagram
    ...
    Worker->>SourceAPI: 2. Fetch data metadata since lastSyncTime
    SourceAPI-->>Worker: Return list of metadata

    Worker->>Redis: 3. Check Sync Confidence
    Redis-->>Worker: Return counter

    alt Destination fetch is NOT skipped
        Worker->>DestAPI: 4. Fetch overlapping data metadata
        DestAPI-->>Worker: Return destination metadata
    end

    Worker->>Worker: 5. Run local conflict resolution on metadata

    alt Records to write > 0
        Worker->>SourceAPI: 6. Hydrate full payloads for required records
        SourceAPI-->>Worker: Return full data payloads
        Worker->>DestAPI: 7. Write final data
        DestAPI-->>Worker: Success
    end

    Worker->>DynamoDB: 8. Update lastSyncTime
    DynamoDB-->>Worker: Success

    Worker->>Redis: 9. Update Sync Confidence counter
    Redis-->>Worker: Success

    Note over SQS, Worker: 10. Lambda service auto-deletes job message on success
    deactivate Worker
```
...
| | `lastSyncTime` | ISO 8601 String | The timestamp of the last successful sync, formatted as `YYYY-MM-DDTHH:mm:ss.sssZ`. |
| **`PushResult` (MVP)** | `success` | Boolean | Whether the entire push operation succeeded. For MVP, this is the only field used. |
| | `totalItems` | Integer | The total number of items attempted. |
| | `pushedItemIds` | Array[String] | A list of source IDs for items successfully pushed. (Post-MVP) |
| | `failedItemIds` | Array[String] | A list of source IDs for items that failed to push. Max size: 100 IDs. (Post-MVP) |
