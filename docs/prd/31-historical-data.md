...
The "Cold Path" architecture for this feature is authoritatively defined in `../prd/45-future-enhancements.md`. **This dependency is critical:** the architecture described in that document must be able to support all functional and non-functional requirements in this PRD, especially the requirement for partial retries (FR-5).
...
***Note on US-31:*** *User story "As a new user (Sarah), I want the app to automatically sync the last 7 days of my data..." was moved to the PRD for New User Onboarding as it describes an initial sync, not a historical data backfill.*

## 3. User Journey & UI/UX

The detailed user journey and flow diagrams are maintained in `../ux/38-ux-flow-diagrams.md`. UI/UX mockups are available in **Figma ([LINK TO BE ADDED])**.
...
4.  **In-Progress:** The app shows a persistent, non-blocking status indicator (e.g., "Importing your history... 25% complete"). The user can leave this screen, use the rest of the app, and optionally cancel the sync.
5.  **Completion:** The user receives a push notification upon completion (success, partial failure, or cancellation).
6.  **Error Handling:** If there are partial failures, the results screen provides a "Retry Failed Items" button. The UI must clearly indicate which date ranges or data types failed.
...
| **FR-2** | System must provide an estimated time to completion. | Based on date range size and provider-specific heuristics. Must be accurate to within +/- 20%. |
| **FR-3** | System must show in-progress status. | Updated via polling the job status endpoint. |
| **FR-4** | System must send a push notification on completion. | `N-05`: "Your historical sync is complete." `N-06`: "Your historical sync finished with some errors." `N-07`: "Your historical sync was canceled." |
| **FR-5** | System must support partial retries. | The backend must track per-chunk success/failure. The UI must allow the user to see which chunks failed and trigger a retry for only those chunks. |
...
| **FR-8** | System must handle subscription changes. | If a user downgrades from Pro to Free while a sync is in progress, the sync job must be gracefully canceled. |
| **FR-9** | User must be able to cancel an in-progress sync. | An API endpoint must be provided to cancel an active Step Functions execution. |
...
| **NFR-3** | Scalability | Concurrent Jobs | > 100 concurrent jobs, each with an average duration of 2 hours and a maximum duration of 8 hours. |
| **NFR-4**| Cost | Cost per 1-year sync | < $1.00 (See `66-costs-model.md` for the cost model). |
...
- [ ] A user can cancel an in-progress historical sync, and the system gracefully stops the job.
...
- [ ] If a user downgrades their subscription while a sync is in progress, the job is canceled.
...
| **R-72** | Hitting third-party API rate limits during large backfills. | High | Medium | Use a configurable concurrency limit in the Step Functions `Map` state (default value: 5). Implement exponential backoff. |
