...
*   **Network Security:** The entire backend compute layer runs within a private Virtual Private Cloud (VPC), providing strong network isolation.
    ...
    *   **Egress:** To control outbound traffic from the VPC to third-party APIs, a **hybrid firewall model** will be implemented, as defined in `06-technical-architecture.md`. This balances cost and security by using separate egress paths depending on the destination's trust level and traffic volume.
