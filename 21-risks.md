...
| **System Overload (R-64)** | - The architecture uses **Amazon SQS queues** as a buffer between the API layer (EventBridge) and the **AWS Fargate worker service**. This queue acts as a shock absorber, durably persisting all incoming jobs.<br>- The Fargate worker service will be configured with an aggressive auto-scaling policy, allowing it to rapidly scale out the number of running tasks based on the SQS queue depth.<br>- This design ensures that even a massive spike in requests or a temporary downstream outage will not lead to lost jobs, only a temporary increase in processing latency. | - The system is designed to handle this automatically. The queue will absorb the load, and the Fargate service will scale out to process it.<br>- SQS queue depth and Fargate service utilization are key metrics on our monitoring dashboard. | A sustained, rapid increase in the `ApproximateAgeOfOldestMessage` metric for the primary SQS queue. |
