...
| **System Overload (R-64)** | - The architecture uses **Amazon SQS queues** as a buffer between the API layer (EventBridge) and the **AWS Fargate worker service**. This queue acts as a shock absorber, durably persisting all incoming jobs.<br>- The Fargate worker service will automatically scale its task count based on the SQS queue depth.<br>- This design ensures that even a massive spike in requests or a temporary downstream outage will not lead to lost jobs, only a temporary increase in processing latency. | - The system is designed to handle this automatically. The queue will absorb the load, and the Fargate service will scale out to process it.<br>- SQS queue depth and Fargate CPU/Memory utilization are key metrics on our monitoring dashboard. | A sustained, rapid increase in the `ApproximateAgeOfOldestMessage` metric for the primary SQS queue. |
...
| **Concurrency Model Feasibility (R-69)** | - **Mandatory PoC:** A proof-of-concept must be executed to validate the Fargate model's cost and performance at peak load.<br>- **Gated Implementation:** Implementation of the full architecture is gated by the successful completion and approval of the PoC. | - If the PoC fails to validate the model, the project's financial model and NFRs must be fundamentally re-evaluated. | This risk is currently active. The trigger for resolution is the successful completion and approval of the PoC. |
